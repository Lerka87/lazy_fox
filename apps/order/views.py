...
def add_to_cart(request):
    ...
    if form.is_valid():
        ...
        breadcrumbs = {'current': 'Додано у кошик'}
        return render(
            request,
            'order/added.html',
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    breadcrumbs = {
                        reverse('cart'): 'Кошик',
                        # reverse('create_order'): 'Оформлення замовлення',
                        'current': 'Замовлення створено'
                    }
                    return render(request, 'order/created.html', {'breadcrumbs': breadcrumbs})
            except Exception as e:
                error = f'Замовлення не створилось. {e}. Напишіть будь ласка нашому менеджеру'
        else:
            error = form.errors
